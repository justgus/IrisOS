...
- Functional: Conch supports `io open`, `io close`, `io send`, `io recv`, `io await` (names TBD).
...
- Validate task IDs, channel/port handles, and payload encodings before invocation.
...
- The representation of I/O handles in Conch session state.
- How to encode bytes payloads in a stable, user-friendly CLI format.
